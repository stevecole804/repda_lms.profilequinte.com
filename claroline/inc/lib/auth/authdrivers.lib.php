        ...
        if ( $userDataList->numRows() > 0 )
        {
            foreach ( $userDataList as $userData )
            {
                if ( $this->password === $userData['password'] )
                {
					...
					. "FROM `{$tbl['tracking_event']}`\n"
					. "WHERE "
					. "user_id = ". $this->userId . " and type = 'user_login'"
					;

					$userLoginCount = Claroline::getDatabase()->query( $sql );
					
					foreach ( $userLoginCount as $loginCount )
					{
						$this->loginCount = $loginCount['loginCount'];
					}
