...
class ClaroBanner extends CoreTemplate
{
    ...
    private function _prepareUserBanner()
    {
        if( claro_is_user_authenticated() )
        {
			...
			$userToolUrlListLeft[]  = '<a href="http://wiki.regioneastpda.ca" target="_blank">Wiki</a>';				
    ...
    private function _prepareCampusBanner()
    {
        ...
        $bannerInstitutionName = (get_conf('institutionLogo') != '')
            ? '<img src="' . get_conf('institutionLogo')
                . '" alt="' . get_lang(get_conf('institution_name')) . '" />'
            : get_lang(get_conf('institution_name'))
            ; 											// modified to allow the institution name to get translated to french by steve cole
