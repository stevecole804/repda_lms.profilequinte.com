...
else
{
    ...
    if (!empty($fileList))
    {
        foreach($fileList as $thisFile )
        {
            ...
            {
                $out .= '<a target="_blank" class="'.$style.' item'.$classItem.'" href="'
                    . claro_htmlspecialchars(Url::Contextualize( $_SERVER['PHP_SELF'] .
                    '?docView=image&amp;file=' . download_url_encode($thisFile['path']) . '&amp;cwd='
                    . $curDirPath . $searchCmdUrl ))
                    .'">'; // This line has been modified by adding a target to the link Steve Cole 
            }
            else
            {
                    $out .= '<a target="_blank" class="'.$style.' item'.$classItem.'" href="'.$urlFileName.'" '.$target.' >';
            } // end if is_image
